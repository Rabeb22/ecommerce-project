...
import { USER_LOGIN_SUCCESS } from '../actionTypes/userConstants';
...
const LoginPage = ({ location, history }) => {
  ...
  const redirect = searchParams ? searchParams.get('login') : '/';
  ...
  useEffect(() => {
    ...
    if (!userInfo && !loading && error) {
      setAuthFailedMsg('Wrong informations verify your email or password');
    }
  }, [navigate, userInfo, loading, error]);

  useEffect(() => {
    if (searchParams.get('login') === 'failed') {
      const errorCodeQuery = searchParams.get('errorCode');
      const errorCode = errorCodeQuery.get('errorCode');
      switch (errorCode) {
        case '0':
          setAuthFailedMsg(
    ...
    if (login === 'success') {
      ...
      axios
        ...
        .then(({ data }) => {
          ...
          navigate('/shipping'); // move to shipping page by default
        });
    }
        ...
        <FormContainer>
          <div className="form-inner-container">
            ...
            {showLoading ? (
              ...
              <div>
                <h3>
                  Please use the link sent to your email and reset your password
                  right away!
                </h3>
              </div>
            ) : (
